...
# Setup task 4/5 (job_type:local).
#
# `srtctl install qwen3-32b-sglang ...` submits a SLURM job that fetches:
#   - Qwen3-32B weights from HF
#   - sglang v0.5.11 container .sqsh
...
INSTALL_NAME="qwen3-32b-sglang"
HF_REPO_ID="Qwen/Qwen3-32B"
MODEL_ALIAS="qwen3-32b"                   # must match config_template.yaml model.path
CONTAINER_IMAGE="lmsysorg/sglang:v0.5.11" # source benchmark uses sglang v0.5.11

# We're running here on the login node (job_type:local) under the LLMB
# workload venv that install_srtslurm.sh editable-installed srtctl into.
# That gives us a login-side `srtctl` to issue the install command —
# which itself just generates and sbatches a wrapper script. We pass
# --venv to point that wrapper at the COMPUTE-side aarch64 venv (also
# built by install_srtslurm.sh via srun), so when the sbatch job runs on
# a Grace node it activates the right interpreter.
...
JOB_ID=$(echo "$INSTALL_OUT" | grep -oE 'squeue -u \$USER -j [0-9]+' | tail -1 | awk '{print $NF}')
if [[ -z ${JOB_ID:-} ]]; then
