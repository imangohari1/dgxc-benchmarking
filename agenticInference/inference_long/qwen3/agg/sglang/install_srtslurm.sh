...
#
#   1. Clone github.com/NVIDIA/srt-slurm @ 0a873270 to
...
if [[ -d "$SRT_SLURM_DIR/.git" ]]; then
    echo "[install_srtslurm] srt-slurm already cloned; fetching and checking out $SRT_SLURM_COMMIT"
    git -C "$SRT_SLURM_DIR" fetch --quiet origin
    git -C "$SRT_SLURM_DIR" checkout "$SRT_SLURM_COMMIT"
else
    echo "[install_srtslurm] cloning srt-slurm → $SRT_SLURM_DIR, then checking out $SRT_SLURM_COMMIT"
    git clone "$SRT_SLURM_REPO" "$SRT_SLURM_DIR"
    git -C "$SRT_SLURM_DIR" checkout "$SRT_SLURM_COMMIT"
fi
